...
$(document).ready(function () {
   ...
   //=======Begin====Initialize Control================
    ...
    var ddlCustomerName1 = $("#ddlCustomerName1");
    ...
    var ddlCustomerName = $("#ddlCustomerName");
    ...
    var txtPDDeposit = $("#txtPDDeposit");
    var ddlMode = $("#ddlMode");
    var txtAssmentAmount = $("#txtAssmentAmount");
    var txtModeNo = $("#txtModeNo");
    var ddlBankName = $("#ddlBankName");
    var txtModeDate = $("#txtModeDate");
    var btnAdd = $("#btnAdd");
    var tblPaymentDet = $("#tblPaymentDet");
    ...
    var divCustomerName = $("#divCustomerName"); 
    ...
    var txtBalanceAmount = $("#txtBalanceAmount");
    var tblPendingInvoiceList = $("#tblPendingInvoiceList");
    var divPendingInvoice = $("#divPendingInvoice");
    var hdnPayFromId = $("#hdnPayFromId");
    var hdnCustomerName = $("#hdnCustomerName");
    var btnShowadjstInvoice = $("#btnAddAdjsutement");
    var ddlTDSWorkyear = $("#ddlTDSWorkyear");
   ...
   //=======End====Initialize Control================
    ...
    ImportReceiptCtrl.ShowAdjstInvoiceButton = btnShowadjstInvoice;
    ImportReceiptCtrl.InvoiceList = tblInvoiceList;
    ImportReceiptCtrl.PaymentFrom = ddlPaymentFrom;
    ImportReceiptCtrl.ReceiptAmount = txtReceiptAmount;
    ImportReceiptCtrl.TDS = txtTDSAmount;
    ImportReceiptCtrl.PDDeposit = txtPDDeposit;
    ImportReceiptCtrl.Mode = ddlMode;
    ImportReceiptCtrl.AssmentAmount = txtAssmentAmount;
    ImportReceiptCtrl.ModeNo = txtModeNo;
    ImportReceiptCtrl.BankName = ddlBankName;
    ImportReceiptCtrl.ModeDate = txtModeDate;
    ImportReceiptCtrl.AddButton = btnAdd;
    ImportReceiptCtrl.PaymentDet = tblPaymentDet;
    ...
    ImportReceiptCtrl.AccountNameFrom = ddlAccount;
    ...
    ImportReceiptCtrl.BalanceAmount = txtBalanceAmount;
    ImportReceiptCtrl.TablePendingInvoiceList = tblPendingInvoiceList;
    ImportReceiptCtrl.DivPendingInvoice = divPendingInvoice;
    ImportReceiptCtrl.HiddenPayFromId = hdnPayFromId;
    ImportReceiptCtrl.HiddenBillPartyId = hdnCustomerName;
    ImportReceiptCtrl.TDSWorkyear = ddlTDSWorkyear;
    ...
    //====Call On Add Button ================
    ImportReceiptCtrl.AddButton.click(function () {
        addPaymentList(ImportReceiptCtrl);
    });
    //====Call On Add Button ================
    ...
    ImportReceiptCtrl.PrintButton.click(function () {
        //
        PrintReceipt(ImportReceiptCtrl);
    });
   ...
   //===========End==Call On TDS Amount Changes Text Box======
    ...
    ////==Start==On Select Billing Customer================
    //ImportReceiptCtrl.CustomerName.change(function () {
    //    BindPendinReceipt(ImportReceiptCtrl.CustomerName.val());
    //});
    ////==End====On Select Billing Customer================

    //==Start==On Select Pending Receipt================
    $("#ddlPendingReceipt").change(function () {
        ShowPendingAmount();
    });
    //==End====On Select Pending Receipt================
});

function PrintReceipt(ControlCtrl) {
    debugger;
    var data1 = { 'receiptNo': ControlCtrl.ReceiptNo.val()};
    data = JSON.stringify(data1);

    $.ajax({
        url: '/ImportReceipt/ImportReceiptPrint',
        type: 'Post',
        data: data,
        async: false,
...
function SaveValidation(ControlCtrl) {
    debugger;
    if ($("#ddlReceiptType").val() != "On Account Receipt" && $("#ddlReceiptType").val() != "Write Off" && $("#ddlReceiptType").val() !="TDS Receipt") {
        if (ControlCtrl.ReceiptAmount.val() == "") {
            alert("Receipt Amount cannot be Blank.");
            ControlCtrl.ReceiptAmount.focus();
            return false;
        }
    }

    if (ControlCtrl.PaymentFrom.val() == "") {
        alert("Please select Payment From");
        ControlCtrl.PaymentFrom.focus();
        return false;
    }

    if ($("#ddlReceiptType").val() != "On Account Receipt Adjustment") {
        if (ControlCtrl.CustomerNameFrom.val() == "") {
            alert("Please select Payment From Customer.");
            ControlCtrl.CustomerNameFrom.focus();
            return false;
        }
    }
...
function ValidateSearchCret(ControlCtrl) {
    ...
    else if (ControlCtrl.SearchCriteria.val() == 'BOENo' || ControlCtrl.SearchCriteria.val() == 'ChequeNumber') {
        ControlCtrl.DivCustomerName.hide();
        ControlCtrl.DivAssessmentNo.hide();
        ControlCtrl.DivBLNo.hide();
...
function DisplayInvoiceList(ControlCtrl) {
        ...
        success: function (data) {
            ...
            $('#tblInvoiceList').dataTable({
                ...
                "rowCallback": function (row, data, dataIndex) {
                    ...
                    ControlCtrl.ReceiptType.attr('disabled', 'disabled');
                    ...
                    //TotalReceiptAmount = parseFloat(TotalReceiptAmount) + parseFloat(data.ReceivedAmount);
                    //TotalTDSAmount = parseFloat(TotalTDSAmount) + parseFloat(data.TDS);
                }
    ...
    BindPendinReceipt(ControlCtrl.HiddenBillPartyId.val());
};

function GetReceiptMode(ControlCtrl) {
    ...
    var PendingReceipt = $("#ddlPendingReceipt").val();
    var data1 = { 'ModeNo': ModeNumber,'PaymentType': PaymentType, 'Amount': ReceiptAmt};
    var data = JSON.stringify(data1);
    $.ajax({
        url: "/ImportReceipt/AddModeVeli",
        data: data,
        type: "POST",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (Data) {
            ...
            if (Data != "") {
                ...
                GetReceiptvalidation(ControlCtrl)
            }
        },
...
function GetReceiptvalidation(ControlCtrl)  {
    ...
    //if (TotalTDSAmount > 0){
        ...
        if ($("#ddlReceiptType").val() != "On Account Receipt") {
...
function addPaymentList(ControlCtrl) {
    var ModeNo = ControlCtrl.ModeNo.val();
    ...
    var data = JSON.stringify(data1);
    $.ajax({
        url: "/ImportReceipt/AddDirectFuel",
        data: data,
        type: "POST",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function Save(ControlCtrl) {
    debugger;
    var validatesearch = $('input[type="search"]').val();
    if (validatesearch != "") {
        alert("Please clear the value from the search box");
        return false
    }

    
    var BalanceAmount = ControlCtrl.BalanceAmount.val();
    if (Validation(ControlCtrl) == false) {
        return false;
    }
    ...
    if ($("#ddlReceiptType").val() != "On Account Receipt Adjustment") {
        if (ControlCtrl.HiddenPayFromId.val() == "" || ControlCtrl.HiddenPayFromId.val() == "0") {
            alert('Payment From Name cannot be Blank.');
            return false;
        }

        if (BalanceAmount > 0) {
            alert('Balance amount cannot be greater then zero.');
            return false;
        }
    }
    var  Account = $("#ddlAccount").val();
    var Search1 = $("#txtBOENo").val();
    if (Account == 0) {
        document.getElementById("ddlAccount").style.borderColor = "red";
    ...
    if ($("#ddlReceiptType").val() == "On Account Receipt Adjustment") {
        ...
        var txtPendingReceiptAmount = $("#txtPendingReceiptAmount").val();
        var txtPendingReceiptAmount = $("#txtPendingReceiptAmount").val();
        var txtSelectedInvoiceAmount = $("#txtSelectedInvoiceAmount").val();
        var PDAReceipt = 0;

        try {

            var splitItem = $("#ddlPendingReceipt").val();
            var SplitAmount = "";
            SplitAmount = splitItem.split('~');
            PDAReceipt = SplitAmount[0];

            if (PDAReceipt <= 0) {
                alert("Please select Valid Receipt No.");
                return false;
            }

        } catch (ex) { alert("Please select Valid Receipt No.");}
       
    }

    if (PDAReceipt == undefined) {
        PDAReceipt = 0;
    }
    ...
    try {
        ...
        //=========================Payment Details List for Save Data Into DataBase================
        var tablearray1 = [];
        var table = document.getElementById("tblPaymentDet");
        var viewModel = objectifyForm($('#ReceiptForm').serializeArray());
        ...
        try {
            for (var i = 1; i < table.rows.length; i++) {
                ...
                row = table.rows[i];
                var BankId = "";
                var PaymentId = row.cells[1].childNodes[1].value;
                var Amount = row.cells[2].innerHTML;
                var ModeNo = row.cells[3].innerHTML;
                try {
                    BankId = row.cells[4].childNodes[1].value;
                } catch (ex) { }

                var ModeDate = row.cells[5].innerHTML;
                tablearray1.push({ 'PaymentId': PaymentId, 'Amount': Amount, 'ModeNo': ModeNo, 'BankId': BankId, 'ModeDate': ModeDate });
            };
        } catch (ex) {}

        
        //============End=============Payment Details List for Save Data Into DataBase=============
        debugger;
        var cnf = confirm('Are you sure you want to save receipt!');
        if (cnf == true) {
            if (parseInt(BalanceAmount) < 0) {
                var r = confirm('Are you sure you want to save receipt with asscess amount !');
                if (r == true) {
                    debugger;
                    $.ajax({
                        type: 'POST',
                        url: '/ImportReceipt/SaveReceiptForm',
                        data: '{viewModel: ' + JSON.stringify(viewModel) + ', invoiceData: ' + JSON.stringify(tablearray) + ',PayFrom: ' + JSON.stringify($("#ddlPaymentFrom").val()) + ',paymentData: ' + JSON.stringify(tablearray1) + ',Category: ' + JSON.stringify(ControlCtrl.CategoryMode.val()) + ',TDSPerc: ' + JSON.stringify(ControlCtrl.DropTDSPercentage.val()) + ',ReceiptType: ' + JSON.stringify(ControlCtrl.ReceiptType.val()) + ',Remarks: ' + JSON.stringify(ControlCtrl.Remarks.val()) + ',PayParty: ' + JSON.stringify(ControlCtrl.HiddenPayFromId.val()) + ',ADVReceiptNo: ' + JSON.stringify(PDAReceipt) + ',BillParty: ' + JSON.stringify(ControlCtrl.HiddenBillPartyId.val()) + ',Search: ' + JSON.stringify(Search1) + ',TDSWorkyear: ' + JSON.stringify(ControlCtrl.TDSWorkyear.val()) + '}',
                        ...
                        success: function (data) {
                            ...
                            {
                                if (data == -1) {
                                    ControlCtrl.ReceiptNo.val(0);
                                    ClearControl(ControlCtrl);
                                    alert('Receipt Already Gegerated.');
                                    return false;
                                }
                                else {
                                    var d = new Date();
                                    ControlCtrl.ReceiptNo.val(data);
                                    ControlCtrl.ReceiptDate.val((d.getDate() + "/" + d.getMonth() + "/" + d.getFullYear() + " " + d.getHours() + ":" + d.getMinutes()));
                                    ControlCtrl.SaveButton.attr('disabled', 'disabled');
                                    alert('Record Saved Successfully');
                                    PrintReceipt(ControlCtrl);
                                    ClearControl(ControlCtrl);
                                }
                ...
                $.ajax({
                    type: 'POST',
                    url: '/ImportReceipt/SaveReceiptForm',
                    data: '{viewModel: ' + JSON.stringify(viewModel) + ', invoiceData: ' + JSON.stringify(tablearray) + ',PayFrom: ' + JSON.stringify($("#ddlPaymentFrom").val()) + ',paymentData: ' + JSON.stringify(tablearray1) + ',Category: ' + JSON.stringify(ControlCtrl.CategoryMode.val()) + ',TDSPerc: ' + JSON.stringify(ControlCtrl.DropTDSPercentage.val()) + ',ReceiptType: ' + JSON.stringify(ControlCtrl.ReceiptType.val()) + ',Remarks: ' + JSON.stringify(ControlCtrl.Remarks.val()) + ',PayParty: ' + JSON.stringify(ControlCtrl.HiddenPayFromId.val()) + ',ADVReceiptNo: ' + JSON.stringify(PDAReceipt) + ',Search: ' + JSON.stringify(Search1) + ',TDSWorkyear: ' + JSON.stringify(ControlCtrl.TDSWorkyear.val()) + '}',
                    //data: JSON.stringify(formData),
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    async: true,
...
function checkItemcount() {
    ...
    var table = $('#tblPaymentDet').DataTable();
...
function ValidateRecAmt(id) {
    ...
    $("#txtSelectedInvoiceAmount").val(parseFloat(totamt));

    ValidateTotalCheckAmount();
}
...
function ChkAmtValidate(id) {
    ...
    $('input[type=checkbox]').each(function (index, value) {
        if (Currentindex == (index + 1)) {
            if (this.checked) {
                ...
                var table = document.getElementById("tblInvoiceList");
                var tablearray = [];
                if ($("#txtTotalSelectedInvoice").val() == "" || $("#txtTotalSelectedInvoice").val() == undefined) {
                    $("#txtTotalSelectedInvoice").val() = "0";
                }
                else {
                    $("#txtTotalSelectedInvoice").val(parseInt($("#txtTotalSelectedInvoice").val()) + 1);
                }
                for (var i = 1; i < table.rows.length; i++) {
                    if (i == Currentindex) {
                        row = table.rows[i];
                        ...
                        if ($("#ddlReceiptType").val() == "On Account Receipt Adjustment") {
                            ...
                            if (txtPendingReceiptAmount == "") {
                                txtPendingReceiptAmount=0;
                            }

                            if (txtSelectedInvoiceAmount == "") {
                                txtSelectedInvoiceAmount=0;
                            }
                            ...
                            if ((parseFloat(txtBalanceAmount) + (parseFloat(ReceivedAmount) - parseFloat(TDSAmount))) > parseInt(txtPendingReceiptAmount)) {
                                alert("Invoice Amount cannot be Greater then total Receipt Amount");
                                $('input[type=checkbox]')[Currentindex-1].checked = false;
                                return false;
                            }
                        }

                        row.cells[9].innerHTML = parseFloat(PrevRecAmount) + parseFloat(ReceivedAmount) - parseFloat(TDSAmount);
                        row.cells[10].innerHTML = parseFloat(ReceivalAmount) - parseFloat(ReceivedAmount);
    ...
    if (totalReceivedAmount > 0) {
        ...
        $("#txtSelectedInvoiceAmount").val(parseFloat(totalReceivedAmount));
    }
    ...
    if (checkboxarray.length == 0) {
        ...
        $("#txtSelectedInvoiceAmount").val(parseFloat(0));
    }
    ...
    $("#ddlCustomerName").autocomplete({
        source: function (request, response) {
            $.ajax({
                ...
                data: "{ 'prefixText': '" + request.term + "','Mode': '" + ControlCtrl.CategoryMode.val() + "'}",
                dataType: "json",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                success: function (data) {
...
function ClearControl(ControlCtrl) {
    debugger;
    ControlCtrl.ReceiptNo.val('');
    ControlCtrl.ReceiptType.val('GeneralReceipt');
    ...
    ControlCtrl.CustomerName.val('');
    ControlCtrl.CustomerNameFrom.val('');
    ControlCtrl.InvoiceList.empty();
    ControlCtrl.PaymentDet.empty();
    ControlCtrl.IGMNo.val('');
    ControlCtrl.ItemNo.val('');
    ControlCtrl.BLNo.val('');
    ControlCtrl.BOENo.val('');
    ...
    ControlCtrl.ModeNo.val('');
    ControlCtrl.AssessmentNo.val('');
    ControlCtrl.AssmentAmount.val(0);
    ControlCtrl.BankName.val('');
    ControlCtrl.ModeDate.val('');
    InitializeTable(ControlCtrl);
    ControlCtrl.DropTDSPercentage.val('4');
    ControlCtrl.SaveButton.removeAttr('disabled');
    ControlCtrl.BalanceAmount.val(0);
    //ControlCtrl.IGMNo.attr('disabled', 'disabled');
    //ControlCtrl.ItemNo.attr('disabled', 'disabled');
    //ControlCtrl.BLNo.attr('disabled', 'disabled');
    //ControlCtrl.BOENo.attr('disabled', 'disabled');
    //ControlCtrl.BondNo.attr('disabled', 'disabled');
    //ControlCtrl.ShippingBillNo.attr('disabled', 'disabled');
    //ControlCtrl.AssessmentNo.attr('disabled', 'disabled');
    //ControlCtrl.CustomerName.removeAttr('disabled');
};

function BindPendinReceipt(CustomerId) {
...
function ShowPendingAmount() {
    ...
    if (parseInt(Amount) > 0) {
        ...
        $("#txtPendingReceiptAmount").val(0);
    }

}

function ValidateTotalCheckAmount() {
    debugger;
    var checkboxarray = [];
    var txtSelectedInvoiceAmount = $("#txtSelectedInvoiceAmount");
    var TotalReceiptAmount = 0;
    var table = document.getElementById("tblInvoiceList");
    ...
    for (var i = 1; i < table.rows.length; i++) {
        ...
        for (var k = 0; k < checkboxarray.length; k++) {
            ...
            if (invoiceno == checkboxarray[k]) {
                ...
                if (NetRecAmount == "" || NetRecAmount == null) {
                    NetRecAmount = 0;
                }

                TotalReceiptAmount += parseInt(NetRecAmount);
            }
        }
    }
...
function DisplayAdjstInvoiceList(ControlCtrl) {
    ...
    $.ajax({
        url: '/ImportReceipt/GetInvoiceListAdjstment',
        type: 'Post',
        data: data,
        async: false,
        contentType: 'application/json; charset=utf-8',
