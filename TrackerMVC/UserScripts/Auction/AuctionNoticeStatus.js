...
$(document).ready(function () {
    ...
    AuctionNoticeStatusCtrl.Remarks = $("#txtRemarks");
    AuctionNoticeStatusCtrl.Reference = $("#txtReference");
    ...
    InitalizeTBL();
    getAuctionUpdatedList();
    AutomCom();
    //=============Beign Call Auction Status List On Show Button Click Event===============
    $("#AuctionStatus").click(function () {
        debugger;
        $("#divAuctionStatusList").show();
        $("#divUpdateAuctionStatus").hide();
        getAuctionUpdatedList();
    });
    //=============End Call Auction Status List On Show Button Click Event===============
...
function Save(ControlCtrl) {
    ...
    if (ControlCtrl.Remarks.val() == "") {
        alert('Please Enter Remarks.');
        ControlCtrl.Remarks.focus();
        return false;
    };
    ...
    $.ajax({
        type: 'POST',
        url: '/Auction/SaveAuctionStatus',
        data: '{IGMNo: ' + JSON.stringify(ControlCtrl.IGMNo.val()) + ', ItemNo: ' + JSON.stringify(ControlCtrl.ItemNo.val()) + ', ContainerNo: ' + JSON.stringify(ControlCtrl.ContainerNo.val()) + ', Remarks: ' + JSON.stringify(ControlCtrl.Remarks.val()) + ', ReferenceNo: ' + JSON.stringify(ControlCtrl.Reference.val()) + '}',
        ...
        success: function (data) {
            if (data == 0) {
                ...
                window.location = "/Auction/AuctionNoticeStatus";
            }
        },
...
function getAuctionUpdatedList() {

    $.ajax({

        url: '/Auction/getAuctionUpdatedStatus',
        type: 'Post',
        data: '',
        async: false,
        contentType: 'application/json; charset=utf-8',
        success: function (data) {
            ...
            RepTableDataJson($("#tblAuctionStatusList"), data.DocumentList, "Auction Status List", "AuctionUpdateList");
        },
        error: function (errormessage) {
            alert(errormessage.responseText);
            $("#tracker-loader").fadeOut("slow");
...
function ClearControl(ControCtrl) {
    ...
    ControCtrl.Remarks.val('');
    ControCtrl.Reference.val('');
    ...
    ControCtrl.Remarks.removeClass("form-control is-invalid state-invalid")
    ControCtrl.Remarks.addClass("form-control")

    ControCtrl.Reference.removeClass("form-control is-invalid state-invalid")
    ControCtrl.Reference.addClass("form-control")
};